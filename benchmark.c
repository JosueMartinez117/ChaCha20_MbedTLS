...
void cypher_message_chacha20(void);
/*******************************************************************************
 * Variables
 ******************************************************************************/
unsigned char message[]={"Test message to encrypt"};
unsigned char test_message[sizeof(message)];
...
void hash_sha256(){
	...
	mbedtls_printf("\r\n--- DEBUG-INFO: hash_sha256 function called  ---\r\n");
	int i;

    mbedtls_printf( "---SHA-256 Implementation started---\r\n" );

    mbedtls_sha256_ret(test_message, BUFSIZE, hash, 0);
    mbedtls_printf("Hash: \r\n");
    for (i = 0; i < HASH_SIZE; i++) {
    	mbedtls_printf("%02x ", hash[i]);
    }
    mbedtls_printf("\r\n");
    mbedtls_printf("\r\n--- DEBUG-INFO: hash_sha256 function completed  ---\r\n");
}

void cypher_message_chacha20(){

    mbedtls_printf("\r\n--- DEBUG-INFO: cypher_message_chacha20 function called  ---\r\n");
	int i;
	int string_len = 23;
	/*
    ...
    //Generate a nonce with random seed
    mbedtls_ctr_drbg_random(&ctr_drbg, nonce, NONCESIZE);

    mbedtls_printf( "\r\n---Chacha20 Implementation started---\r\n" );
    mbedtls_printf( "\n---Encrypting message---\n\r" );
    mbedtls_chacha20_crypt(key, 0U, 0U, BUFSIZE, message, test_message);

    mbedtls_printf("\r\n--- Uncrypted message:\r\n");
    for (i = 0; i < string_len; i++) {
    	mbedtls_printf("%c", message[i]);
    }
    mbedtls_printf("\r\n");

    mbedtls_printf("\r\n--- Encrypted message:\r\n");
    for (i = 0; i < BUFSIZE; i++) {
    	mbedtls_printf("%02x ", test_message[i]);
    }
    mbedtls_printf("\r\n");

    mbedtls_printf("\r\nReleasing memory used for random number generation:\r\n");
    mbedtls_ctr_drbg_free(&ctr_drbg);

    mbedtls_printf("\r\n--- DEBUG-INFO: cypher_message_chacha20 function completed  ---\r\n");
}
